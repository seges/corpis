...
import sk.seges.sesam.service.remote.IDataRemoteLoader;
 ...
 * @author eldzi
 */
public class PagedDTOList<E> extends AbstractPagedDTOList<E> {
	private IDataRemoteLoader<List<E>> dataLoader;
	...
	public void setDataLoader(IDataRemoteLoader<List<E>> dataLoader) {
		this.dataLoader = dataLoader;
	}

	@Override
	protected final PagedResultDTO<List<E>> getPagedResult() {
		if (pagedResult == null) {
	 ...
	 */
	private void fetchPage(PageDTO page) {
	...
	public Iterator<E> iterator() {
	   ...
	   private class Itr implements Iterator<E> {
		...
		public void remove() {
			...
			try {
				PagedDTOList.this.remove(lastRet);
				if (lastRet < cursor)
					cursor--;
				lastRet = -1;
